...
async function login_handler(request, response, context)
{
    ...
    try
    {
        ...
        if (context.sessions) {
            context.sessions.set(sessionId, { user: user, date: new Date() });
        }
...
async function logout_handler(request, response, context)
{
    ...
    if (cookies) {
        const sessionId = cookies.split('=')[1]; // Simplificado
        if (context.sessions) {
...
// v4: Handler para las acciones protegidas de la v3
async function action_handler(request, response, context)
{
    const url = getRequestUrl(request, context.config);
    const path = url.pathname;
    
    // El request.user fue inyectado por el dispatcher en server.mjs
    ...
    if (!allowed)
    {
        ...
        response.end(JSON.stringify({ 
            error: `Acceso denegado al endpoint ${path}`, 
            authorized: false 
        }));
        return;
    }
    ...
    const messages = {
        '/print': 'Acción print ejecutada correctamente.',
        '/log': 'Log registrado en el sistema.',
        '/help': 'Ayuda: Los endpoints disponibles son /print, /log, /help, /sayHello, /sayBye',
        '/sayHello': '¡Hola desde el servidor v4!',
        '/sayBye': '¡Adiós! Sesión de prueba finalizada.'
    };
    ...
    response.end(JSON.stringify({
        ...
        authorized: true,
        action: path,
        message: messages[path] || 'Acción ejecutada'
    }));
}
