...
{
    const sql = "INSERT INTO user (username, password_hash, email, role_id) VALUES (?, ?, ?, ?) RETURNING id, username, email, role_id, created_at";
...
{
    const sql = "SELECT id, username, password_hash, email, role_id FROM user WHERE username = ?";
    const stmt = db.prepare(sql);
...
{
    const sql = "UPDATE user SET username = ?, password_hash = ?, email = ?, role_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? RETURNING id, username, email, role_id, updated_at";
    const stmt = db.prepare(sql);
...
// v4: Función de autorización acoplada de la v3 usando el esquema de la v2
function authorize(db, userId, endpointPath)
{
    ...
    `;
    const row = db.prepare(sql).get(userId, normalizedPath);
